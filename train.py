...
from torch.utils.data import DataLoader
from models.model import Generator, Discriminator, QNet
import torch.nn as nn
import torch.optim as optim
from utility.utils import weights_init, QNetLoss, sample_c
from torch.autograd import Variable
import torchvision.utils as vutils


parser = argparse.ArgumentParser()
parser.add_argument('--dataset', default="mnist", required=False, help='cifar10 | lsun | mnist |imagenet | folder | lfw | fake')
...
if __name__ == "__main__":
    ...
    fixed_noise = torch.cat([fixed_z, fixed_c], 1)
    ...
    for epoch in range(opt.niter):
        for i, data in enumerate(dataloader, 0):
            ...
            z = Variable(torch.randn(opt.batchSize, nz-10, 1, 1)).to(device)
            c = sample_c(opt.batchSize).to(device)
            ...
            ############################
            # (3) Update G and Q network: maximize entroph
            ###########################
            netG.zero_grad()
            netQ.zero_grad()
            output = netQ(fake.detach())
            errQ_fake = qnetloss(c, output)
            errQ_fake.backward()
            optimizerQ.step()
            optimizerG.step()
